  ...
  return <main className="min-h-screen bg-[var(--ink)] text-[var(--paper)]">
    ...
    <section className="mx-auto max-w-7xl px-5 pb-24 pt-5 sm:px-8 lg:px-12">
      <div className="grid gap-8 lg:grid-cols-[1.1fr_.9fr] lg:items-end">
        <div><div className="mb-3 flex items-center gap-2 text-xs font-bold uppercase tracking-[.18em] text-[var(--acid)]"><Sparkles className="size-4" /> Feed builder</div><h1 className="font-display text-[clamp(3rem,6vw,6rem)] leading-[.9] tracking-[-.055em]">Turn a playlist into<br /><span className="text-white/35">a better rabbit hole.</span></h1></div>
        <p className="max-w-xl text-base leading-7 text-white/55 sm:text-lg">Jev sorts up to 100 videos three ways: go deep, ship something, or cut the slop. Share the result as a clean, no-login feed.</p>
      </div>
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1.35fr)_minmax(320px,.65fr)]">
        ...
        <aside><div className="sticky top-6 space-y-4">{top && <section className="overflow-hidden rounded-[1.75rem] bg-[var(--paper)] p-6 text-[var(--ink)]"><div className="flex items-center justify-between"><p className="text-xs font-bold uppercase tracking-[.16em] text-black/40">Jev signals</p><span className="rounded-full bg-[var(--ink)] px-3 py-1 text-xs text-white">{templateDefinitions[template].shortName} lens</span></div><p className="mt-7 font-display text-4xl leading-none">Why it ranks here.</p><p className="mt-3 text-sm leading-6 text-black/55">{top.reason}. Signals: {top.signals.map((item) => item.toLowerCase()).join(", ")}.</p><div className="mt-6 grid gap-3 rounded-2xl bg-[var(--ink)] p-4 text-white">{template === "stretch" ? <><Metric label="Concept density" value={top.depth} accent /><Metric label="Rabbit-hole pull" value={top.curiosity} /><Metric label="Prerequisite load" value={top.difficulty} /><Metric label="Signal / noise" value={top.focus} /></> : template === "balanced" ? <><Metric label="Build payoff" value={top.buildValue} accent /><Metric label="Entry speed" value={top.learnability} /><Metric label="Explanation signal" value={top.clarity} /><Metric label="Signal / noise" value={top.focus} /></> : <><Metric label="Signal / noise" value={top.focus} accent /><Metric label="Concept density" value={top.depth} /><Metric label="Explanation signal" value={top.clarity} /><Metric label="Build payoff" value={top.buildValue} /></>}</div><div className="mt-5 grid grid-cols-3 gap-2 text-center text-xs"><div><p className="font-display text-xl">{top.difficulty}</p><p className="text-black/40">Prereqs</p></div><div><p className="font-display text-xl">{top.curiosity}</p><p className="text-black/40">Pull</p></div><div><p className="font-display text-xl">{top.score}</p><p className="text-black/40">Rank</p></div></div><Button onClick={() => setSelected(top)} className="mt-6 h-12 w-full rounded-xl bg-[var(--ink)] text-white"><Play className="fill-current" /> Watch the top pick</Button></section>}
          <section className="rounded-[1.75rem] border border-white/10 bg-white/[0.045] p-6"><button onClick={() => setShowLogic((value) => !value)} className="flex w-full items-start gap-3 text-left"><div className="grid size-10 shrink-0 place-items-center rounded-full bg-[var(--violet)]/25 text-violet-300"><ShieldCheck className="size-5" /></div><div className="flex-1"><h3 className="font-display text-xl">Three modes. Three answers.</h3><p className="mt-1 text-sm leading-6 text-white/45">Choose what you want from this rabbit hole.</p></div><ChevronDown className={`mt-2 size-4 text-white/35 transition ${showLogic ? "rotate-180" : ""}`} /></button>{showLogic && <div className="mt-5 space-y-3 border-t border-white/10 pt-5 text-sm leading-6 text-white/50"><p><strong className="text-white/80">Deep Dive:</strong> What is worth understanding all the way down?</p><p><strong className="text-white/80">Ship Mode:</strong> What gets me from idea to working code?</p><p><strong className="text-white/80">No Slop:</strong> What respects my attention?</p></div>}<div className="mt-5"><div className="mb-2 flex justify-between text-xs"><span>Playlist completed</span><span>{progress}%</span></div><Progress value={progress} className="h-2 bg-white/10 [&_[data-slot=progress-indicator]]:bg-[var(--acid)]" /></div></section>
        </div></aside>
      </div>
    </section>
  </main>;
