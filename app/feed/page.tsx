...
export default function PublishedFeed() {
  ...
  return <main className="min-h-screen bg-[var(--ink)] text-[var(--paper)]">
    ...
    <section className="mx-auto max-w-7xl px-5 pb-24 pt-10 sm:px-8 lg:px-12">
      <div className="grid gap-8 lg:grid-cols-[1fr_.78fr] lg:items-end"><div><div className="mb-3 flex items-center gap-2 text-xs font-bold uppercase tracking-[.18em] text-[var(--acid)]"><Sparkles className="size-4" /> Ordered by Jev</div><h1 className="font-display text-[clamp(3rem,6vw,5.5rem)] leading-[.9] tracking-[-.05em]">{playlist.title}</h1><p className="mt-4 text-white/45">{playlist.channel} · {videos.length} videos</p></div><div className="rounded-2xl border border-white/10 bg-white/5 p-5"><p className="text-xs font-bold uppercase tracking-[.16em] text-white/35">How this playlist is ordered</p><p className="mt-2 font-display text-2xl">{templateDefinitions[template].name}</p><p className="mt-2 text-sm leading-6 text-white/45">{templateDefinitions[template].description}</p></div></div>
